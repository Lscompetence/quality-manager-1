...
- ✅ **Sprint 5** — Mini-apps groupe 3 (6 complexes) : Check-list site, Check-list examen, **Cockpit C7** (KPI), **Matrice compétences** (grille 2D), **Grille évaluation** (calculs auto), **Suivi assiduité** (émargements + alertes)
- ⏭ **Sprint 6** — Vue Documents, Paramètres organisme, Notifications, tests Vitest, seed data

**Architecture mini-apps** : on a deux types
- **`kind: "generic"`** : rendues par le moteur générique `<MiniApp schema={...} />`. Tout est déclaratif. 15 mini-apps suivent ce pattern.
- **`kind: "custom"`** : rendues par un composant React dédié dans `components/miniapps/custom/`. Pour les UI métier spécifiques (KPI dashboards, grilles 2D, calculs pondérés). 4 mini-apps (Cockpit C7, Matrice compétences, Grille évaluation, Suivi assiduité).

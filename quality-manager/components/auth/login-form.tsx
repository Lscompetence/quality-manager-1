...
export function LoginForm({ portal = "admin" }: { portal?: LoginPortal }) {
  ...
  const onSubmit = async (data: LoginInput) => {
    setPending(true);
    const result = await login(data, portal);
    if (result && !result.ok) {
      ...
      <div className="space-y-5">
        <div>
          ...
          <Input
            id="email"
            type="email"
            placeholder={portal === "client" ? "vous@entreprise.fr" : "prenom.nom@votre-of.fr"}
            autoComplete="email"
            {...register("email")}
          />

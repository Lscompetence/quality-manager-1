...
export function AttachmentList({
  ...
  /** Masque le bouton de suppression — pour un espace client, en lecture + depot uniquement. */
  canDelete?: boolean;
  /** Masque « Ajouter une preuve » — consultation seule (mini-apps côté client). */
  canAdd?: boolean;
}) {
    ...
    <div>
      {attachments.length === 0 ? (
        ...
        <div className="space-y-2 mb-4">
          {attachments.map((att) => (
            <AttachmentRow key={att.id} att={att} onChange={() => router.refresh()} canDelete={canDelete} />
          ))}
        </div>
      )}
...
function AttachmentRow({
  ...
  const handleDelete = () => {
    startTransition(async () => {
      ...
      toast.success(`« ${att.file_name} » supprimé`);
      onChange();
    });
  };
      ...
      {canDelete && (
        <>
          ...
          <ConfirmDialog
            ...
            description={
              <>
                <b className="font-semibold text-foreground">{att.file_name}</b> sera retiré du dossier et le fichier

  ...
  const { error } = await supabase.from("miniapp_data").upsert(
    {
      ...
      // Les données de mini-app sont un JSON libre par construction (chaque
      // mini-app a son propre schéma) : la colonne est un JSONB, non typé
      // plus précisément côté base.
